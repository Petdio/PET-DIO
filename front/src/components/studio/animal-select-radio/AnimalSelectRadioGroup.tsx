...
function AnimalSelectRadioGroup({ animalItems, onSelect }: Props) {
  ...
  return (
    <Grid width="100%" container padding="1rem" justifyContent="space-between">
      {animalItems.map((animalItem, idx) => (
        <Grid key={idx} item xs={5.8} onClick={() => handleSelect(idx)}>
          <AnimalSelectButton
            content={animalItem}
